...
class InsertHandler(webapp2.RequestHandler):
    def post(self):
        ...
        cursor.execute("insert into course \
            (course_code,course_name,course_description,credit_lecture,credit_lab,credit_learning,price,prerequisite,department,faculity,faculity_id) VALUES ('%s','%s','%s','%d','%d','%d','%d','%d','%s','%s','%d')"%
            (data_code,data_course_name,data_course_description,data_credit_lecture,data_credit_lab,data_credit_learning,total,data_prerequisite,data_department_full,data_faculity,data_faculity_id))
        conn.commit()
        ...
        cursor3.execute("insert into log\
            (staff_id,course_id,day,time,type) values(2,(select course_id from course where course_code = '%s'),CURDATE(),CURTIME(),1)"%
            (data_code))        
        conn3.commit()
